...
in
{
  options.braunse.wsl = with types; {
    ...
    enableWslDns = mkEnableOption "WSL DNS";
  ...
  config =
    mkIf cfg.enable (mkMerge [
      {
        braunse.wsl.wslConf.config = {
          network = {
            hostname = mkDefault config.networking.hostName;
            generateResolvConf = toString cfg.enableWslDns;
          };
        };
        ...
        networking.resolvconf.enable = !cfg.enableWslDns;
